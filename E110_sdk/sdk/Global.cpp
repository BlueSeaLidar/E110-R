...
int LD::parse_data(int len, unsigned char *buf, RawDataHdr_LD *dat, int &consume, char *result)
{
	...
	for (idx = 0; idx <= len - 12; idx++)
	{
		if (buf[idx] == 0x54 && buf[idx + 1] == 0x2c)
		{
			...
			int tmp = dat->end_angle - dat->start_angle;
			int span = tmp > 0 ? tmp : tmp + 36000;
			...
			for (unsigned int i = 0; i < dat->N; i++)
			{
				...
				float tmp = (dat->start_angle + i * angle_interval);
				dat->points[i].angle = (tmp > 36000) ? (tmp - 36000) : tmp;
			}
...
int LD::whole_data_process(RawDataHdr_LD raw, FrameData_LD &framedata, FrameData_LD &tmpdata, std::string &error)
{
	bool iscross = false;
	int crossidx = raw.N;
	if (raw.start_angle > raw.end_angle)
	{
		iscross = true;
		for (int i = 1; i < raw.N; i++)
		{
			if (raw.points[i].angle < 18000)
			{
				crossidx = i;
				break;
			}
	...
	if (raw.end_angle == 0 || raw.end_angle == 36000)
		iscross = true;
	...
	{
		memcpy(&framedata.points[framedata.N], &raw.points[i], sizeof(LidarPoint_LD));
		// printf("%s %d %d %d %d\n",__FUNCTION__,__LINE__,framedata.points[framedata.N].angle,i,framedata.N);
		framedata.N++;
	}
	for (int i = crossidx; i < raw.N; i++)
	{
		memcpy(&tmpdata.points[tmpdata.N], &raw.points[i], sizeof(LidarPoint_LD));
		// printf("%s %d %d %d %d\n",__FUNCTION__,__LINE__,tmpdata.points[tmpdata.N].angle,i,tmpdata.N);
		tmpdata.N++;
	}

	// 丢包校验
	if (framedata.N > 0 && iscross)
	{
		for (int i = 1; i < framedata.N; i++)
		{
			float currentAngle = framedata.points[i].angle;
			float lastangle = framedata.points[i - 1].angle;
			float diff = (currentAngle - lastangle > 0) ? (currentAngle - lastangle) : (currentAngle - lastangle + 36000);
			if (diff > 200)
			{
				char tmp[512] = {0};
			...
			// printf("%s %d %d %d %d\n",__FUNCTION__,__LINE__,currentAngle,i,framedata.N);
		}
		return 1;
	}
	...
	for (int i = 0; i < nfan; i++)
	{
		...
		for (int j = 0; j < raw->N; j++)
		{
			...
			if (raw->points[j].angle < framedata.points[framedata.N - 1].angle)
			{
				...
				if (raw->points[j].angle > 100)
				{
					printf("drop data %d %d  %f  %f %d %d %d\n", i, j, raw->points[j].angle, framedata.points[framedata.N - 1].angle, raws[i].N, framedata.N, nfan);
					framedata.N = 0;
					tmpraw.N = 0;
					nfan = 0;
					continue;
